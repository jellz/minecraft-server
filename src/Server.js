...
class Server extends EventEmitter {
  ...
  */
  ...
  handleLogin(client) {
    ...
    this.emit('playerLogin', client);

    client.on('packet', (data, meta) => {
      if (meta.name === 'chat') return this.emit('chat', player, data.message);
      if (meta.name === 'arm_animation')
        return this.emit('arm', player, data.hand);
      if (meta.name === 'settings') return this.emit('playerSettings', player, data);
    });
